...
library(nlme)
library(tidyverse)
data_original <- read_delim('ClassAssessment.txt')
data <- data_original
...
data$fittedVals <- Pedagogy_lm$fitted.values
data$Residuals <- Pedagogy_lm$residuals
...
# GLS model
```{r}
Pedagogy_gls <- gls(model= Final ~ Semester + NStudents + Exam1 + Exam2 + Exam3 + HW + Quiz, data= data, weights=varFixed(~1/NStudents), method="ML")
