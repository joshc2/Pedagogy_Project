...
Pedagogy_lm <- lm(Final ~ Quiz, data = data)

bptest(Pedagogy_lm)


# FoodExpenses_gls <- gls(model=EatingOut~Income, data=data, weights=varExp(form=~Income), method="ML")
# Do we need to standardize and have then have var = 1
?gls
...
standard <- resid(object=web.gls, type="pearson")
